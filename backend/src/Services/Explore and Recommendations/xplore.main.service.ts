...
export const exploreService = async (params: ExploreAndRecommendParams) => {
  const { includeCreators, includeGroups, tags, limit = 20, offset = 0 } = params;
  ...
  if (tags && tags.length > 0) {
    results.posts = results.posts.filter((p) =>
      p.tags?.some((t: any) => tags.includes(t.tag?.name ?? ""))
    );

    results.groups = results.groups.filter((g) =>
      g.members?.some((m: any) =>
        m.user?.interests?.some((i: any) =>
...
export const recommendationService = async (params: ExploreAndRecommendParams) => {
  ...
  if (orientation)
    filtered = filtered.filter((u) => u.orientation === orientation);

  // --- Filter by shared interests ---
  if (interests && interests.length > 0) {
    filtered = filtered.filter((u) =>
      u.interests?.some((i: any) =>
        i.interest && interests.includes(i.interest.name ?? "")
      )
    );
  }
